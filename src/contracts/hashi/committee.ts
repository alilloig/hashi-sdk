...
const $moduleName = 'hashi::committee';
...
export const Committee = new MoveStruct({ name: `${$moduleName}::Committee`, fields: {
...
export const CommitteeSignature = new MoveStruct({ name: `${$moduleName}::CommitteeSignature`, fields: {
        epoch: bcs.u64(),
        signature: bcs.vector(bcs.u8()),
        signers_bitmap: bcs.vector(bcs.u8())
    } });
export function CertifiedMessage<T extends BcsType<any>>(...typeParameters: [
    T
]) {
    return new MoveStruct({ name: `${$moduleName}::CertifiedMessage<${typeParameters[0].name as T['name']}>`, fields: {
...
export interface NewCommitteeSignatureOptions {
    package?: string;
    arguments: NewCommitteeSignatureArguments | [
        epoch: RawTransactionArgument<number | bigint>,
        signature: RawTransactionArgument<number[]>,
        signersBitmap: RawTransactionArgument<number[]>
    ];
