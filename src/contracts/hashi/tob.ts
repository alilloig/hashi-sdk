...
const $moduleName = 'hashi::tob';
export const TobKey = new MoveStruct({ name: `${$moduleName}::TobKey`, fields: {
        epoch: bcs.u64(),
        batch_index: bcs.option(bcs.u32())
    } });
export const ProtocolType = new MoveEnum({ name: `${$moduleName}::ProtocolType`, fields: {
        Dkg: null,
        KeyRotation: null,
        NonceGeneration: null
    } });
export const EpochCertsV1 = new MoveStruct({ name: `${$moduleName}::EpochCertsV1`, fields: {
        epoch: bcs.u64(),
        protocol_type: ProtocolType,
        /** Dealer submissions indexed by dealer address (first-submission-wins). */
        certs: linked_table.LinkedTable(bcs.Address)
    } });
export const DealerMessagesHashV1 = new MoveStruct({ name: `${$moduleName}::DealerMessagesHashV1`, fields: {
        dealer_address: bcs.Address,
        messages_hash: bcs.vector(bcs.u8())
    } });
export const DealerSubmissionV1 = new MoveStruct({ name: `${$moduleName}::DealerSubmissionV1`, fields: {
