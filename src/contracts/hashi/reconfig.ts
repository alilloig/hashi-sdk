...
const $moduleName = 'hashi::reconfig';
export const ReconfigCompletionMessage = new MoveStruct({ name: `${$moduleName}::ReconfigCompletionMessage`, fields: {
        /** The epoch of the new committee. */
        epoch: bcs.u64(),
        /** The MPC committee's threshold public key. */
        mpc_public_key: bcs.vector(bcs.u8())
    } });
export const StartReconfigEvent = new MoveStruct({ name: `${$moduleName}::StartReconfigEvent`, fields: {
        epoch: bcs.u64()
    } });
export const EndReconfigEvent = new MoveStruct({ name: `${$moduleName}::EndReconfigEvent`, fields: {
        epoch: bcs.u64(),
        /** The MPC committee's threshold public key. */
        mpc_public_key: bcs.vector(bcs.u8())
    } });
export const AbortReconfigEvent = new MoveStruct({ name: `${$moduleName}::AbortReconfigEvent`, fields: {
        epoch: bcs.u64()
    } });
...
export interface StartReconfigOptions {
    package?: string;
    arguments: StartReconfigArguments | [
        self: RawTransactionArgument<string>
    ];
}
...
export interface EndReconfigOptions {
    package?: string;
    arguments: EndReconfigArguments | [
        self: RawTransactionArgument<string>,
        mpcPublicKey: RawTransactionArgument<number[]>,
        signature: RawTransactionArgument<number[]>,
        signersBitmap: RawTransactionArgument<number[]>
...
export interface AbortReconfigOptions {
    package?: string;
    arguments: AbortReconfigArguments | [
        Self: RawTransactionArgument<string>
    ];
}
